...
loadDateUtils = function () {
        ...
        if (!this.sheet) {
          ...
          this.sheet.getRange("A1:N2").setValues([
            [
              ...
              "残業申請", "残業取消", "有給休暇", "無給休暇", "リクエストが拒否されました"
            ],
            [
              ...
              "<@#1> #2を残業として申請しました", "<@#1> #2の残業を取り消しました",
              "<@#1> #2を有給休暇として登録しました", "<@#1> #2を無給休暇として登録しました", "<@#1> #2リクエストが拒否されました",
            ]
          ]);
        }
      }
  ...
  loadGSTimesheets = function () {
    ...
    GSTimesheets.prototype.get = function (username, date) {
      ...
      return ({ user: username, date: row[0], signIn: row[1], signOut: row[2], note: row[3], totalWorkTime: row[4], dayOffWorking: row[4], weekdayOvertime: row[4], weekendOvertime: row[4], holidayOvertime: row[4], month: row[4], paidLeaveMonth: row[4], unpaidLeaveMonth: row[4], });
    };
  
    // set data to sheet
    GSTimesheets.prototype.set = function (username, date, params) {
      var row = this.get(username, date);
      _.extend(row, _.pick(params, 'signIn', 'signOut', 'note', 'totalWorkTime', 'dayOffWorking', 'weekdayOvertime', 'weekendOvertime', 'holidayOvertime', 'month', 'paidLeaveMonth', 'unpaidLeaveMonth'));
  
      var sheet = this._getSheet(username);
      var rowNo = this._getRowNo(username, date);
      var dayOff = DateUtils.parseWday(sheet.getRange("B2").getValue());
      var month = DateUtils.format("m", date);
  
      // check day off working
      if (_.contains(dayOff, DateUtils.toDate(date).getDay())) {
        row.dayOffWorking = '週末休日'
      } else {
        row.dayOffWorking = ''
      }
  
      var data = [DateUtils.toDate(date), row.signIn, row.signOut, row.note, row.totalWorkTime, row.dayOffWorking, row.weekdayOvertime, row.weekendOvertime, row.holidayOvertime, row.month, row.paidLeaveMonth, row.unpaidLeaveMonth]
        .map(function (v) {
          return v == null ? '' : v;
        });
      ...
      sheet.getRange("G" + rowNo).setFormula('=IF(E' + rowNo + '<>"", IF(D' + rowNo + '="残業申請",IF(F' + rowNo + '="", IF(E' + rowNo + '-8/24>0,E' + rowNo + '-8/24,0),""),""),"")');
  
      // set for weekendOvertime - 残業時間（週末）
      sheet.getRange("H" + rowNo).setFormula('=IF(E' + rowNo + '<>"", IF(D' + rowNo + '="残業申請",IF(F' + rowNo + '="週末休日", E' + rowNo + ',""),""),"")');
  
      // set for holidayOvertime - 残業時間（祝日）
      sheet.getRange("I" + rowNo).setFormula('=IF(E' + rowNo + '<>"", IF(D' + rowNo + '="残業申請",IF(F' + rowNo + '="祝日休日", E' + rowNo + ',""),""),"")');
  
      // set for month - 残業時間
      sheet.getRange("J" + rowNo).setFormula('=if(A' + rowNo + '="","",if(B' + rowNo + '="", "", "' + month + '"))');
  
      // set for paid leave month - 計上月(有給)
      sheet.getRange("K" + rowNo).setFormula('=if(A' + rowNo + '="","",if(REGEXMATCH(D' + rowNo + ',"有給休暇"), "' + month + '", ""))');
  
      // set for unpaid leave month - 計上月(無給)
      sheet.getRange("L" + rowNo).setFormula('=if(A' + rowNo + '="","",if(REGEXMATCH(D' + rowNo + ',"無給休暇"), "' + month + '", ""))');
  
      // format data
      sheet.getRange("A" + rowNo).setNumberFormat('YYYY/MM/DD')
      sheet.getRange("B" + rowNo + ":C" + rowNo).setNumberFormat('hh:mm')
      sheet.getRange("E" + rowNo).setNumberFormat('hh:mm')
      sheet.getRange("G" + rowNo + ":I" + rowNo).setNumberFormat('hh:mm')
  ...
  function setUp() {
    ...
    if (!global_settings.get('spreadsheet')) {
      ...
      // 休日を設定 (iCal)
      var calendarId = 'en.vietnamese.official#holiday@group.v.calendar.google.com';
  ...
  //   miyamoto.receiver.receiveMessage({ user_name: "nguyenducthang327", text: "おはようございます 9/30 8:30" });
  //   // miyamoto.receiver.receiveMessage({ user_name: "nguyenducthang327", text: "出勤" });
  ...
  // function checkout(e) {
  //   var miyamoto = init();
  //   miyamoto.receiver.receiveMessage({ user_name: "nguyenducthang327", text: "退勤" });
  // }
  ...
  loadTimesheets = function (exports) {
    ...
    Timesheets.prototype.receiveMessage = function (username, message) {
      ...
      if (this.datetime !== null) {
        dateTmp = new Date(this.datetime);
        this.dateStr = DateUtils.format("Y/m/d", this.datetime);
        this.datetimeStr = DateUtils.format("Y/m/d H:M", this.datetime);
      }
  
      if (!this.date) this.date = [dateTmp.getFullYear(), dateTmp.getMonth() + 1, dateTmp.getDate()];
    ...
    // 有給休暇
    Timesheets.prototype.actionOffPaid = function (username, message) {
      if (this.date) {
    ...
    // 無給休暇
    Timesheets.prototype.actionOffUnPaid = function (username, message) {
      if (this.date) {
        var dateObj = new Date(this.date[0], this.date[1] - 1, this.date[2]);
        var data = this.storage.get(username, dateObj);
        if (!data.signOut || data.signOut === '-') {
    ...
    */
    ...
    Timesheets.prototype.actionCancelOff = function (username, message) {
      if (this.date) {
        var dateObj = new Date(this.date[0], this.date[1] - 1, this.date[2]);
        this.storage.set(username, dateObj, { note: '休暇取消' });
        this.responder.template("休暇取消", username, DateUtils.format("Y/m/d", dateObj));
      }
    };
  
    // 残業申請
    Timesheets.prototype.actionOTRequest = function (username, message) {
      if (this.date) {
        var dateObj = new Date(this.date[0], this.date[1] - 1, this.date[2]);
        var data = this.storage.get(username, dateObj);
  
        this.storage.set(username, dateObj, { note: '残業申請' });
        this.responder.template("残業申請", username, DateUtils.format("Y/m/d", dateObj));
      }
    };
  
    // 残業取消
    Timesheets.prototype.actionCancelOTRequest = function (username, message) {
      if (this.date) {
        var dateObj = new Date(this.date[0], this.date[1] - 1, this.date[2]);
        var data = this.storage.get(username, dateObj);
        this.storage.set(username, dateObj, { note: '残業取消' });
        this.responder.template("残業取消", username, DateUtils.format("Y/m/d", dateObj));
      }
    };
